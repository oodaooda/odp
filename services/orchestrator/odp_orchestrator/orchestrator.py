...
class Orchestrator:
    ...
    async def cancel_task(self, project_id: UUID, task_id: UUID) -> bool:
        """Cancel an in-progress task by forcing it to ROLLBACK."""
        t = await self.get_task(project_id, task_id)
        if not t or t.state in (TaskState.COMMIT, TaskState.ROLLBACK):
            return False
        await self._transition(t, TaskState.ROLLBACK)
        await self.memory.write_memory_event(
            project_id=project_id, task_id=task_id,
            type_="decision", actor="user",
            payload={"action": "cancel", "previous_state": t.state},
        )
        return True
        ...
        try:
            while True:
                ...
                if t.state == TaskState.VALIDATE:
                    ...
                    # Post GitHub status checks for each gate (if configured).
                    await self._maybe_post_github_statuses(t, ok2, ok3, ok4)
                    ...
                    if ok2 and ok3 and ok4 and ok5:
                        ...
                        if merge_ok:
                            await self._maybe_create_github_pr(t)
                    else:
                        await self._transition(t, TaskState.ROLLBACK)
                    continue
    ...
    async def _maybe_post_github_statuses(self, t: Task, eng_ok: bool, qa_ok: bool, sec_ok: bool) -> None:
        """Post GitHub commit statuses for each gate if configured."""
        if os.getenv("ODP_GITHUB_STATUS_CHECKS", "0") != "1":
            return
        repo = os.getenv("ODP_GITHUB_REPO", "")
        if not repo:
            return
        try:
            from .github import post_status
            branch = f"odp/task-{str(t.task_id)[:8]}"
            # Use branch name as pseudo-SHA; real impl would resolve HEAD of branch.
            for ok, ctx in [(eng_ok, "odp/engineer"), (qa_ok, "odp/qa"), (sec_ok, "odp/security")]:
                await post_status(
                    repo=repo, sha=branch,
                    state="success" if ok else "failure",
                    context=ctx,
                    description=f"Gate {'passed' if ok else 'failed'}",
                    target_url=f"/projects/{t.project_id}/tasks/{t.task_id}",
                )
        except Exception:
            pass  # Best-effort; don't fail the task over GitHub API errors.

    async def _maybe_create_github_pr(self, t: Task) -> None:
        """Auto-create a GitHub PR after successful commit if configured."""
        if os.getenv("ODP_GITHUB_AUTO_PR", "0") != "1":
            return
        if os.getenv("ODP_AGENT_TEST_MODE", "0") == "1":
            return
        repo = os.getenv("ODP_GITHUB_REPO", "")
        ...
        try:
            ...
            if t.description:
                body_parts.append(f"\n{t.description}\n")
            body_parts.append("## Gate Evidence")
            for key in t.gate_decisions:
                raw = await self.store.get_json(key)
                if raw:
                    gd = GateDecision.model_validate(raw)
                    status = "PASS" if gd.passed else "FAIL"
                    body_parts.append(f"- **{gd.phase}**: {status} — {gd.reason}")
            body_parts.append(f"\n---\nGenerated by ODP task `{t.task_id}`")

            result = await create_pr(
                repo=repo, title=t.title,
                body="\n".join(body_parts),
                head=branch,
            ...
            if result:
                ...
                uri = self._write_artifact_file(
                    t.project_id, t.task_id, "github_pr",
                    f"PR #{result.number}: {result.html_url}\n",
                )
                await self.memory.record_artifact(
                    project_id=t.project_id, task_id=t.task_id, type_="report", uri=uri,
                )
                await self.bus.emit(t.project_id, t.task_id, "github_pr_created", {
                    "pr_url": result.html_url, "pr_number": result.number,
                })
        except Exception:
            pass  # Best-effort.
